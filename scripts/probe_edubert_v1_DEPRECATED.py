# DEPRECATED: circular probe (skill visible in input -> scratch ~99.9%). Use probe_edubert_v2.py.
# Linear probing of a (frozen) EduBERT encoder for SKILL-IDENTITY decodability.
# Question answered: how linearly-decodable is skill identity from each
# pretraining source's representations? Higher probe accuracy = the encoder
# encodes more skill-relational structure. Comparing scratch / in-domain /
# EdNet / Junyi gives mechanistic evidence for what each source captures, to
# correlate with downstream transfer.
#
# Method: freeze encoder, run encode() to get per-interaction hidden states
# (B,L,d), train a single Linear(d -> num_skills+1) probe on the TRAIN split to
# predict skill[t] at each real position, evaluate top-1 accuracy on TEST.
# (Skill identity is part of the input, so a randomly-initialized encoder gives
# a non-trivial baseline; the INTEREST is the GAP between sources and how the
# probe accuracy tracks downstream transfer, not the absolute number.)
#
# Usage (scratch = random encoder baseline; pretrained = load a ckpt):
#   python scripts/probe_edubert.py --processed_dir ../processed/assist2017 \
#       --init scratch --seed 42 --run_type probe_scratch --wandb
#   python scripts/probe_edubert.py --processed_dir ../processed/assist2017 \
#       --init pretrained --encoder_ckpt ../checkpoints/edubert_ednet_pretrain_full_encoder.pt \
#       --seed 42 --run_type probe_ednet --wandb
...
def main():
    ...
    ap.add_argument("--run_type", default="probe")
    ...
    print(f"device={device}  num_skills={num_skills}  run={run_name}  "
          f"init={args.init}  seed={args.seed}")
    ...
    probe = nn.Linear(args.d_model, num_skills + 1).to(device)  # skill-identity probe
    opt = torch.optim.Adam(probe.parameters(), lr=args.lr)
    ce = nn.CrossEntropyLoss(ignore_index=0)  # 0 = PAD skill
    ...
    if args.wandb:
        try:
            ...
            wb_mod.init(project="StudentBERT", name=run_name,
                        config={"task": "probe_skill_identity", "init": args.init,
    ...
    def rep(batch):
        ...
        with torch.no_grad():
            h = backbone.encode(sk, co, tb, key_padding_mask=~mask)  # (B,L,d)
        return h, sk, mask

    def run(split_loader, train=False):
        probe.train() if train else probe.eval()
        correct_n, total_n, loss_sum, nb = 0, 0, 0.0, 0
        for batch in split_loader:
            h, sk, mask = rep(batch)
            logits = probe(h)                       # (B,L,K+1)
            tgt = sk.clone(); tgt[~mask] = 0        # PAD -> ignore_index
            loss = ce(logits.view(-1, logits.size(-1)), tgt.view(-1))
            if train:
                opt.zero_grad(); loss.backward(); opt.step()
            pred = logits.argmax(-1)
            real = mask
            correct_n += int(((pred == sk) & real).sum())
            total_n += int(real.sum())
            loss_sum += float(loss); nb += 1
        return loss_sum / max(nb, 1), correct_n / max(total_n, 1)
    ...
    print(f"\n=== probe (skill-identity) ({run_name}, init={args.init}) ===")
