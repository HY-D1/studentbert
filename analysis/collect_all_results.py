...
# Section 1 is no longer carried as a constant. It is read from the run
...
def render_md(coll):
    ...
    W("- Scratch control (6 seeds at every K including 512, the K=512 cell added 2026-08-11, jobs 9081378-9081383): 0.6347, 0.6476, 0.6423, 0.6523, 0.6586, 0.6597, 0.6675 at K=10..512. It is NOT monotone. From K=320 to K=512 scratch gains +0.0078 while full gains +0.0327 and skill_only +0.0331, and correct_only gains +0.0056, less than scratch. The endpoint divergence is therefore objective-specific and not a generic data-quantity effect. At K=512 correct_only (0.6655) falls BELOW scratch (0.6675). The skill_only minus correct_only gap is negative at every K through 320 (pps 3.14) and positive only at K=512 (pps 4.32); the sign change is bracketed between pps 3.14 and 4.32, NOT near 1.5 (at K=160, pps 1.57, the gap is still -0.0046). Truncation keeps the most recent K interactions per learner, so it lowers total interactions, time horizon and skill composition together; density is not isolated.")
...
def main():
    ...
    ap.add_argument("--inventory", default="run_inventory.tsv",
    ...
    coll["baseline_table"] = baseline_table_from_inventory(args.inventory)
